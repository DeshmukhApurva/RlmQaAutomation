...
And(/^I delete mapping record for field "(.*?)" from "(.*?)" mapping$/) do |toFieldName, mappingComponentNo|
  begin
    if all(".pbSubsection")[1 + (mappingComponentNo.to_i)].has_selector?(:xpath,"//td[text()='#{toFieldName}']")

      within all(".pbSubsection")[1 + (mappingComponentNo.to_i)] do
        find(:xpath, "//td[text()='#{toFieldName}']").find(:xpath, '..').first(:link,"Delete").click
        puts "I clicked delete"
        sleep 5
        puts "#{toFieldName} field has been deleted"
      end
    else if all(".pbSubsection")[1 + (mappingComponentNo.to_i)].has_selector?(:xpath,"//td/span[text()='#{toFieldName}']")
        within all(".pbSubsection")[1 + (mappingComponentNo.to_i)] do
          find(:xpath, "//td/span[text()='#{toFieldName}']").find(:xpath, '../..').first(:link,"Delete").click
          puts "I clicked delete link"
          sleep 5
          puts "#{toFieldName} field has been deleted"
        end
      else
        puts "#{toFieldName} field was not present"
      end
      all(:xpath, "//*/input[@type='button']")[0].click
      sleep 5
      puts "Mapping changes saved"
    end
  rescue Exception => ex
    putstr "Error occurred while verifying Opportunity Fields."
    putstr_withScreen  ex.message
...
And(/^I delete Opportunity Fields "(.*?)"$/) do |arg1|
  begin
    ...
    if fieldPresent > 0
      ...
      puts "#{arg1} field was not present"
    end
    first(".pbButtonb").first(".btn").click
