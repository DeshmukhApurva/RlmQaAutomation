...
And(/^I clone the existing renewal relationship$/) do
  begin
    $url = URI.parse(current_url)
    first(:xpath,'//div/div[2]/table/tbody/tr[2]/th/a').click
    #find(:xpath, "//th[text()='Relationship ID']").find(:xpath, '..').all(:xpath, "following-sibling::tr[2]/th/a")[1].click
    sleep 5

    $old_RR_value = find(:xpath,'//td[text()="Relationship ID"]/following-sibling::td[1]').text
    first(:button,'Clone').click
    sleep 5
    first(:button,'Save').click
    sleep 5
    $new_RR_value = find(:xpath,'//td[text()="Relationship ID"]/following-sibling::td[1]').text
  end
end

And (/^I verify the cloned renewal relationship id is different from the existing renewal relationship id$/) do
  begin
    sleep 5
    
    if $old_RR_value.to_s != $new_RR_value.to_s
      puts "Renewal Relationship is getting cloned with unique ID value"
    end
