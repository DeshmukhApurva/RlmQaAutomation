...
Then (/^I select the "(.*?)"$/) do |data|
  begin
    ...
    sleep 2
    find(:xpath, '//input[contains(@placeholder,"Search...")]').set arg["Name"]
    sleep 2
    find(:xpath, '//input[contains(@id,"phSearchButton")]').click
    sleep 8
    within(".opportunityBlock") do
      within(".list") do
        if first("tbody").all(".dataRow")[0].all("th")[0].first("a").text == arg["Name"]
          first("tbody").all(".dataRow")[0].all("th")[0].first("a").click
          puts "Opportunity #{arg["Name"]} Found..."
        else
          puts "Opportunity #{arg["Name"]} Not Found..."
        end
      end
    end
    sleep 10
    ...
    puts "Error in opening the Opportunity: #{data}"
