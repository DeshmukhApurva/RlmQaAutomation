...
And (/^I verify the changes after sync for "(.*?)" Partner Opportunity$/) do|data|
  begin
    ...
    #within all('.detailList')[0] do
      ...
      $masterOppStageValue = page.find(:xpath,'//td[contains(text(),"Stage")]/following-sibling::td/div').text
    ...
    within all('.listRelatedObject')[0] do
      ...
      tr.each do |row|
        ...
        if row.all(".dataCell")[0].text == arg1[data]
          ...
          "Partner Opportunity #{arg1[data]} and #{row.all('.dataCell')[0].text} does not match"  
        end
      end
    end
  rescue Exception=> ex
